...
def compute_crop_calendar(
    ...
    if Mode == 1:  # Growth in calendar days
        ...
        def Biomas_ini_es(Bio_mul,TopStress,Ksccx_temp,Ksexpf_temp,fcdecline_temp,Kswp_temp):
            ...
            return Bio_cur,Bio_up,Kc_Tr_es,Ks_Tr,Ksc_Total

        _,_,_,_,Ksc_Total=Biomas_ini_es([-1],-1,1,1,0,1)
        ...
        if crop.need_calib==1 or crop.need_calib==-1:
            ...
            while adj_flag and kk_<100:
                ...
                if crop.need_calib==-1 and kk_==1:
                    fcdecline_temp=crop.fcdecline
                    Ksexpf_temp=crop.Ksexpf
                    Kswp_temp=crop.Kswp
                ...
                while adj_wp and k_<100:
                    #print('wp')
                    k_+=1
                    bio_cur,_,_,_,_=Biomas_ini_es(Bio_mul,TopStress,Ksccx_temp,Ksexpf_temp,fcdecline_temp,Kswp_temp)
                    #print(Kswp_temp)
                ...
                if (int(bio_cur/Bio_top*100)==int(crop.RelativeBio*100)) or (kk_>=100 and np.abs(int(bio_cur/Bio_top*100)-int(crop.RelativeBio*100))<5):
                    ...
                    #if unable to calibrate, then return the defaul value (no soil fertility)
                    ...
                    for i in range(0,100):
                        ...
                        bio_temp,_,_,_,_=Biomas_ini_es(Bio_mul,crop.TR_ET0_fertstress*(1-crop.sf_es[i+1]),Ksccx_temp,Ksexpf_temp,fcdecline_temp,Kswp_temp)
            ...
            while possible_flag:
                ...
                for Ksexpf in np.arange(1,101,2)/100:
                    for fcdecline in np.arange(0,100,2)/100/100:
                        ...
                        _,Bio_up,Kc_Tr_es,Ks_Tr,_=Biomas_ini_es(Bio_mul,TopStress,crop.Ksccx_in,Ksexpf,fcdecline,1.0)
